---
title: NeLV
createTime: 2026/01/08 12:28:13
permalink: /notes/Papers/400pegr6/
---
# 新一代无人机大语言模型：从自然语言到自主飞行
...
(i) 作为解析器的 LLM 根据人类飞行员描述的无人机任务（如森林火灾监视或超市物资配送）确定节点 $v \in V$ 及其属性 $X$ 和类型 $Y$。一个特定的森林区域或超市店铺构成一个节点 $v \in V$。这些节点具有不同的属性 $x \in X$，包括地理坐标（经纬度）、运营状态和拥堵程度。节点类型 $y \in Y$ 代表指定的飞行模式和操作方式，例如飞机可能为超市配送执行空中投送，而机场则支持常规的起降程序。
...
其中 $v_i \in V$ 且 $N \ge 2$ 是一个整数，代表路线中的节点数量。路线 $\xi$ 必须包含起飞和降落节点，这两个节点可以是同一个。此处的路线规划是多目标的，始终考虑任务完成度、飞行质量和成本之间的平衡。

(iii) 路径规划器基于初步路线 $\xi$ 生成一个更详细、更具体的路径：



$$\pi = [v_1, p_{1,2}^{(1)}, p_{1,2}^{(2)},..., v_2, p_{2,3}^{(1)}, p_{2,3}^{(2)},..., v_N]$$



其中 $p_{i,i+1}^{(j)}$ 代表连接节点 $v_i$ 到节点 $v_{i+1}$ 的路径段中的第 j 个航路点。这种详细的路径规划综合考虑了 FAA 对各类空域的规定。

(iv) 控制平台将路径转化为可执行的轨迹：



$$\rho = [V_1, p_{1,2}^{(1)}, p_{1,2}^{(2)},..., V_2, p_{2,3}^{(1)}, p_{2,3}^{(2)},..., V_N]$$



其中每个 $V_i$ 代表一个与节点 $v_i$ 的节点类型 $y_i \in Y$ 相对应的专业化空中交通模式。这些模式包括特定的程序，如起飞序列、盘旋点和降落程序，这些都是根据每个节点的操作环境量身定制的。
...
作为解析器的 LLM 是 NELV 系统最基础的组件，因为它定义了无人机飞行图 $G$，其准确性直接影响所有后续流程的结果。尽管我们设想未来的大语言模型不仅能定义图 $G$，还能确定路线 $\xi$、规划路径 $\pi$ 并生成可执行轨迹 $\rho$，但我们发现，由于缺乏在无人机规划数据集上训练的专业化大语言模型，目前的通用大语言模型（如 GPT-4o ）尚不具备足够的能力。因此，我们目前仅将大语言模型用作指令解析器，从自然语言指令中提取基本信息，包括出发点、目的地坐标和任务规格 。
...
在作为解析器的 LLM 构建了图 $G$ 之后，下一步是在 $G$ 中找到一条从起点 $v_1$ 到终点 $v_N$ 且满足任务要求的路线。这些关键点在图 $G$ 中表示为节点 $v \in V$。路线规划的基本目标是基于多个目标确定一个节点序列作为路线 $\xi = [v_1, v_2,..., v_N]$，例如根据燃油成本和消耗情况确定最佳加油地点，同时满足时间限制等各种约束。

在数学上，这个优化问题可以表述为：



$$\xi^* = \arg \min_{\xi \in \Omega} \mathcal{O}_{route}(\xi)$$
...
其中 $\xi^*$ 代表最优路线，$\Omega$ 表示从起点到终点的可行路线空间，$\mathcal{O}_{route}(\xi)$ 描述了控制路线优化过程的目标函数。$\mathcal{L}(v_i, v_{i+1})$ 表示连续节点间的成本函数，包含了空中交通密度、盛行风况、燃油消耗等因素。$\mathcal{R}(v_{i+1})$ 表示访问节点 $v_{i+1}$ 相关的奖励或效用。参数 $\alpha$ 平衡了最小化成本和最大化奖励之间的权衡。$C(\xi)$ 是一个约束函数向量，其上限为 $C^{max}$，涵盖了各种操作限制。$M(\xi)$ 表示任务完成指示函数，如果所有任务要求都得到满足，则等于1，否则为0 。

这些函数由无人机规格、任务参数和操作环境决定，并根据具体要求逐案配置。例如，在执行需要中途加油的远程多跳中转任务时，成本函数 $\mathcal{L}(v_i, v_{i+1})$ 包含了不同机场的燃油价格差异和与飞行相关的消耗指标，而当燃油质量不构成操作相关因素时，奖励函数 $\mathcal{R}(v_{i+1})$ 的值可能微不足道。约束函数 $C(\xi)$ 代表当前的燃油容量水平和航程限制，而任务完成标准 $M(\xi)$ 则表示成功的加油操作和任务目标的实现。关于每个用例的各函数实现的详细描述见章节 4。
...
在路线规划器确定了最优路线 $\xi^*$ 之后，路径规划器会生成一个更详细、更具体的路径 $\pi$，该路径考虑了一整套服务数据 $S$，包括天气预报、地面风险、空域限制和各种监管参数，以确保在最小化操作成本的同时遵守 FAA 的规定。给定路线 $\xi^*$ 中的节点，路径规划问题涉及寻找中间航路点 $p_{i,i+1}^{(j)} \in \mathbb{R}^3$（代表经度、纬度和高度），以构建一条优化复合目标函数的路径。对于连接节点 $v_i$ 和 $v_{i+1}$ 的每个航段，路径规划问题可表述为：



$$\pi_{i,i+1}^* = \arg \min_{\pi \in \Pi_{i,i+1}} \mathcal{O}_{path}(\pi)$$

$$\mathcal{O}_{path}(\pi) = \beta_c \cdot \Phi(\pi) + \beta_f \cdot \mathcal{F}(\pi)$$



其中 $\pi_{i,i+1}^*$ 表示节点 $v_i$ 和 $v_{i+1}$ 之间的最优路径段，$\Pi_{i,i+1}$ 是这些节点间所有可行路径的集合，而 $\mathcal{O}_{path}(\pi)$ 是根据服务数据 $S$ 评估的路径规划目标函数。$\beta_c$ 和 $\beta_f$ 分别是 $\Phi(\pi)$ 和 $\mathcal{F}(\pi)$ 的系数。$\Phi(\pi)$ 量化了路径上的约束违规情况，$\mathcal{F}(\pi)$ 表示在该路径段上评估的成本函数。$\beta_c$ 远大于 $\beta_f$ 以确保约束得到满足。与路线规划（其解空间有限，节点数量有限）不同，路径规划存在无限多的潜在解和无数可能的航路点。因此，约束被直接纳入目标函数，而不是像公式 (4) 中那样分开处理 。

同样，路径规划也高度依赖于具体情况，取决于无人机的类型、尺寸、重量和任务场景。因此，成本函数 $\mathcal{F}(\pi)$ 和约束函数 $\Phi(\pi)$ 都根据具体要求而变化。举例来说，成本函数 $\mathcal{F}(\pi)$ 通常包含与无人机操作相关的多个因素，包括路径长度、任务持续时间、与天气相关的危险以及与地面相关的风险。天气风险通常源自简化预报结冰潜势（SFIP）、对流有效位能（CAPE）和整体理查森数（BRN）等指标，这些指标分别描述了结冰条件、湍流和雷暴的可能性。地面风险是根据飞越区域的人口密度和通过这些区域的轨迹长度来计算的。此外，还可以利用风力信息来优化任务时间，以减少燃油消耗。

完整的 оптимальная 路径 $\pi^*$ 是通过连接路线 $\xi^*$ 中连续节点之间的最优路径段 $\pi_{i,i+1}^*$ 构成的，即 $\pi^* = v_1^* \oplus \pi_{1,2}^* \oplus v_2^* \oplus \pi_{2,3}^* \oplus \dots \oplus \pi_{N-1,N}^* \oplus v_N^*$，其中 $\oplus$ 表示路径连接。为了在包含离散和连续变量的大搜索空间内有效且高效地确定 $\pi^*$，我们采用了粒子群优化（PSO）算法 ，这是一种非常适合此类问题的元启发式算法。

在路径规划应用中，PSO 首先随机生成一组候选路径，共同构成初始种群 $\mathcal{P}_0$。该种群根据以下更新方程进行 $T$ 代迭代演化：



$$v_{t+1} = w v_t + c_1 r_p \cdot (p_t - \mathcal{P}_t) + c_2 r_g \cdot (g_t - \mathcal{P}_t)$$

$$\mathcal{P}_{t+1} = \mathcal{P}_t + v_{t+1}$$



其中 $t=1,2,...,T$ 表示迭代代数。参数 $w, c_1, c_2$ 分别代表惯性权重、个体影响和社会影响，是用于调整 PSO 的超参数。$p_t$ 代表每个粒子历史上达到的个体最佳位置，$g_t$ 表示在第 $t$ 代所有粒子中找到的全局最佳解。向量 $r_p$ 和 $r_g$ 包含在区间  内均匀分布的随机数。这种基于 PSO 的路径规划器的详细算法实现在算法 4 中给出 。
...
为了将最优路径 $\pi^*$ 转化为适合无人机部署的真实任务剖面，起始航路点 $v_1$ 和终端航路点 $v_N$ 必须分别替换为标准化的起飞和降落环路模式。基于 $v_1$ 和 $v_N$ 的坐标，通过地理空间匹配算法识别出相应的机场。随后，从 AirNav 数据库  中检索所有跑道端点的坐标和朝向，并用于计算跑道中心点 $(\varphi_s, \lambda_s)$，该中心点作为环路生成的参考原点。给定预定义的高度规格和航路点间隔参数，使用算法 5 系统地生成一个局部环路模式。
...
环路航路点生成的测地计算始于角距离归一化，其中 $d_k$ 是连续航路点 $k$ 和 $k+1$ 之间的大圆距离（米）：



$$\delta_k = \frac{d_k}{6.371 \times 10^6}$$



其中 $\delta_k$ 是由地球半径（$6.371 \times 10^6$ 米）归一化的角距离。利用这个归一化的角距离 $\delta_k$ 和从航路点 $k-1$ 到航路点 $k$ 的方位角 $\theta_k$，通过球面三角学计算每个航路点的经纬度坐标：

$$ \varphi_k = \arcsin(\sin \varphi_{k-1} \cos \delta_k + \cos \varphi_{k-1} \sin \delta_k \cos \theta_k) $$

$$ \lambda_k = \lambda_{k-1} + \text{atan2}(\sin \theta_k \sin \delta_k \cos \varphi_{k-1}, \cos \delta_k - \sin \varphi_{k-1} \sin \varphi_k) $$

其中 $\varphi_k$ 表示第 $k$ 个航路点的纬度，$\lambda_k$ 表示经度。图 3 展示了一个使用  制作的示例性环路配置，演示了这些测地变换。最后，系统地附加特定任务的操作指令，例如在指定航路点周围进行顺时针盘旋以完成货物投放或装载操作，从而构建最终的可执行轨迹 $\rho$，该轨迹随后被上传到控制平台以进行自主任务执行 。
